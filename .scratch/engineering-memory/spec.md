...
| Auto-build skill | **Auto-build** | `auto-build` | User-global | `~/.agents/skills/auto-build/` |

No optional sync skill. No project copies of the rule or Loop skills. Explore subagent is invoke-only (Architecture Review), not an Install drop. Package-owned on-demand skills (core-four + auto-build pipeline including `wayfinder`, `to-tickets`, `drain-tickets`, grill aliases) ship as **globals** (always overwrite with Loop refresh) — not project Store drops; invocation stays on-demand. Closed set + ownership: [`skills/DEPENDENCIES.md`](../../skills/DEPENDENCIES.md). Interactive `/grilling` and `/wayfinder` stay HITL unless the parent is `/auto-build` (AFK auto-accept). Planning maps remain outside the Memory Store.

**Global Loop updates:** Memory Install is the sole updater (no upgrade skill; Review / always-on never self-refresh). Every invoke (1) **always replaces** the closed global set (`engineering-memory` rule, `memory-install`, `improve-codebase-architecture`, plus all package-owned deps in DEPENDENCIES.md) with packaged latest from the invoke’s **package root** (package-owned; no global merge; fail loudly on partial global write — do not start project writes), then (2) runs project preflight/merge. Project conflict does **not** roll back globals. No globals-only flag. **Project Store + AGENTS index never auto-upgrade** when stubs advance — ours/skip, missing/create, conflict/offer interactive merge. Version pins and release changelogs deferred. Detail: [issues/08-global-skill-updates.md](issues/08-global-skill-updates.md).
...
Exact mechanics (detail: [issues/13-plan-sized-architecture-handoff.md](issues/13-plan-sized-architecture-handoff.md)); always-on rule keeps a pointer only (ticket 12).
